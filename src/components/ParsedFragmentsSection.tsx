...
const ParsedFragmentsSection: React.FC<ParsedFragmentsSectionProps> = ({ parsedUrl }) => {
          ...
          <div className="flex flex-wrap gap-4">
            {Object.entries(parsedUrl.query).map(([key, value]) => (
              <div key={key} className="flex items-center gap-1">
                <span className="url-fragment-query px-2 py-1 rounded font-mono text-sm">{key}</span>
                <span className="text-gray-400">=</span>
                <span className="url-fragment-query px-2 py-1 rounded font-mono text-sm">{value}</span>
              </div>
            ))}
          </div>
        </div>
